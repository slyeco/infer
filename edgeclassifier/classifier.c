...
void preprocess_input(double *input) {
    ...
    // Step 1: Compute log10(si + 1) for each element in the input
    for (int i = 0; i < 10; i++) {
        log_input[i] = log10(input[i] + 1);
    }

    // Step 2: Compute differences D0 to D8 and store them back in the input array
    for (int i = 0; i < 9; i++) {
        input[i] = log_input[i + 1] - log_input[i];
    }
}
